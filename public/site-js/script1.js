(function($) {
  ...
  $(document).ready(function() {
    ...
    $("#block-slider").owlCarousel({
      ...
      singleItem: true,
      navigation: true,
      navigationText: ["<span class='icon-left-open-big'></span>",
    ...
    for (var i = 0; i < backgroundImage.length; i++) {
      if (backgroundImage.eq(i).attr("data-background")) {
        backgroundImage.eq(i).css("background-image", "url(" +
          backgroundImage.eq(i).data("background") + ")");
      }
    }
    ...
    $('.search-form i').on("click", function() {
      ...
      if ($(this).closest('.search-form').find('input[type="text"]').val()) {
        $(this).closest('.search-form').find('input[type="submit"]').trigger(
          'click');
      }
    });
